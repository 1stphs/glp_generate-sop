...
**在第二阶段，Reviewer 的核心使命是“骨架健壮度核验”；Curator 的核心使命是“提取经验，沉淀为规则”。**
*   **沙盘推演 (Simulator)**：拿着真实的 Excel 数据，去强行代入（试填）阶段一骨架预留的插槽。如果发现 Excel 中存在特定分组或维度，但在骨架中找不到容身之“槽”，立刻抛出结构缺失隐患。
*   **骨架核验 (Reviewer)**：基于 Simulator 的推演报告，核对：**骨架预留的插槽和结构，是否足以承载这份真实 Excel 的全部逻辑深度？**如果 Excel 拆分了批内/批间数据，或包含溶血特异性评价，但生成的骨架却只有一个笼统的 `[精密度接受标准]` 插槽或根本没留溶血位置，Reviewer 直接予以打回。
*   **规则提炼 (Curator)**：Curator 接收 Failure Cause，生成具体的**结构修复规则指令**更新至该章节专属的 `rules.json`（如要求“必须补充对批内/批间的拆分描述插槽”）。
...
graph TD
    ...
    Phase_Router -->|Phase 2: 真实数据压测与骨架进化| DataSync[注入Excel数据 & V1_Draft骨架]:::phase2
    End1 -.-> DataSync
    Mapper --> DataSync
...
*   `Simulator` 增加“假想执行”职能：拿着沙盒解析出的真实的 Excel 数据，去尝试代入（填涂）骨架草稿中的插槽。如果发现 Excel 中存在某些数据维度但在骨架中根本找不到容身之“槽”，则将其作为隐患抛出。
*   `Reviewer` 修改裁决标准：不再检查具体数值是否填对，而是基于 Simulator 的试运行报告，核对“骨架中预留的插槽体系，是否足以承载这份真实数据的全部分支”。如果不够，直接打回。
