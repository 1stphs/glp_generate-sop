...
1. **从“通用全局修正”到“章节级专家”**：抛弃用一个万能 [writer_skill](file:///Users/pangshasha/Documents/github/glp_generate-sop/sop_deeplang/memory_manager.py#171-255) 应对所有错误的做法。全局Skill只负责如语气、Markdown格式等“通用底层逻辑”。新增**章节级规则库（Chapter-specific Rule Library）**，其产物不仅在内部引导 AI，也将**显性地呈现在 SOP 文档的首部**，作为具体的“核心填写规则”。
1.1 **产物形态演进**：最终生成的 SOP 模板不再是一个光秃秃的插槽文件，而是一份包含 **“一、核心填写规则”**（教人怎么填）与 **“二、通用标准化模板”**（标准样板房）的完整工艺资产。
2. **安全隔离的Excel解析**：Excel解析模块以“沙盒函数（Sandbox）”形式存在，独立于核心LLM链路。它纯代码驱动，只负责输出结构化的数据（Markdown表格、JSON片段），避免核心链路受外部格式不稳定影响。
3. **两阶段解耦与增量进化**：
    *   **阶段一 (冷启动/Bootstrap)**：基于单份资料快速“盲写”出附带占位符的雏形骨架。
    *   **阶段二及以后 (增量泛化/Generalization)**：接入真实、多样的 Excel/Report 验证源，对现有骨架进行持续“撞击测试”。通过不断丰富“核心填写规则”来增强系统的泛化兼容性，而非频繁大幅改写“通用模板”。

---

## 2. 阶段一：形式导向的基础 SOP 生成（Phase 1 - Form-Oriented Baseline）

**核心思想**：**“冷启雏形，快速占位”**。第一阶段的任务并非直接产出一份绝对精确的终稿，而是利用大模型强大的语言泛化能力，根据一份基础资料（Report+Protocol），快速搭建出结构合规、话术标准的**带有插槽（Placeholder）的初始泛化 SOP**。
...
## 3. 阶段二：增量压测与规则持续进化（Phase 2 - Incremental Generalization）

**目标**：**“博采众长，以规则驭变化”**。利用多份（新加入的）真实报告、方案以及**Excel 结果数据**，对阶段一生成的骨架进行循环“撞击测试”。
*   **进化核心**：主要针对 **“## 一、核心填写规则”** 进行无限增量式更新（如：当新项目出现了内标物，则规则库增加一条内标的取值逻辑）。
*   **稳定性维护**：**“## 二、通用模板”** 保持高频复用和微小波动，确保 SOP 的格式一致性和泛化性，防止因单一项目的特殊性而导致模板“改动过大”、“面目全非”。
**特点**：强迭代、知识沉淀导向，最终产物是具备极高鲁棒性的“专家级规程模板”。
...
*   **规则提炼 (Curator)**：Curator 接收 Failure Cause，评估这是“个别偏差”还是“共性逻辑分支”。如果是后者，则更新该章节专属的 `rules.json`。**此时系统会优先丰富“核心填写规则”章节**，指导 Writer 在保留现有模板核心话术的前提下，仅对插槽体系进行微调优化。这样既增强了泛化性，又守住了稳定性。
...
【请根据最新追加的规则与参照的复杂数据结构，生产本章节的终极产物，必须包含以下两部分内容：】

1. ## 一、核心填写规则
(将本章节专属 rules.json 中的逻辑转化为人类可读的填槽指南。如：1. 具体基质：根据方案中 3.1 节内容填写...)

2. ## 二、通用标准化模板
(排版极致精密的插槽正文，确保插槽形式与规则中定义的维度死死咬合。)
```
...
graph TD
    ...
    Phase_Router -->|Phase 2: 实战数据压测与全局骨架进化| DataSync[注入 Excel/Report/Protocol & V1_Draft]:::phase2
    End1 -.-> DataSync
    Mapper --> DataSync
...
```markdown
## 一、核心填写规则
1. **具体基质**：根据方案（Protocol）中的具体基质名称进行填写。
2. **待测物正式名称**：根据方案中的 Analyte 正式名称进行填写，需注意中英文全称对照。
3. **标准化输出**: 输出内容形式与下方模板示例保持绝对一致，不做额外的补充说明。

## 二、通用标准化模板
方法学验证结果表明，该方法符合生物样品测定要求，可用于 [具体基质] 中 [待测物正式名称] 浓度的测定。
```

<!-- 注意看里面有完整的方案和报告，还有新加的要修改的方案文件，都是docx形式的，结果数据都是excel形式。我现在想为每一个报告文件维护一个解析后的文件，里面把每个章节的方案和报告都解析成md格式（注意保留表格和公式，章节哟啊合并报告和方案，如果有一方确实缺失内容，填入“无数据”即可），excel也按表格按sheet解析出来成md（应该在sandbox文件夹里有解析的脚本了）。这样得到完整的能传入程序的数据。请你为我写好脚本，做好文件维护，不要太混乱。 -->