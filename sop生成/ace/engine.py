...
from .core import LocalPlaybookStorage, ContextEnhancer, ApprovalInterceptor
...
class ACE:
    ...
    def __init__(
        ...
        # Initialize three-layer storage memory
        playbook_path = os.path.join(os.getcwd(), "agent_memory", "playbooks", "base_rules.json")
        self.storage = LocalPlaybookStorage(playbook_path)
        self.enhancer = ContextEnhancer(self.storage)
        self.interceptor = ApprovalInterceptor()
        
        # Initialize playbook (backward compatibility)
        if initial_playbook:
            self.playbook = initial_playbook
        else:
            self.playbook = self.enhancer.get_relevant_playbook("")
    ...
    def _train_single_sample(
        ...
        # Retrieve relevant playbook dynamically
        current_playbook = self.enhancer.get_relevant_playbook(question + "\n" + context)
        ...
        log_bullet_usage(usage_log_path, epoch, step, task_dict, bullet_ids,
                       playbook=current_playbook, is_correct=is_correct)
        ...
        tracking_dict = {
            "pre_train_result": {
                ...
                "playbook_num_tokens": count_tokens(current_playbook),
                "playbook_length": len(current_playbook)
            }
        }
        ...
        if not is_correct:
            ...
            for round_num in range(max_num_rounds):
                ...
                if bullet_tags:
                    self.storage.update_metrics(bullet_tags)
                    current_playbook = self.enhancer.get_relevant_playbook(question + "\n" + context)
        ...
        if step % curator_frequency == 0:
            ...
            _, self.next_global_id, operations, _ = self.curator.curate(
            ...
            if self.use_bulletpoint_analyzer and self.bulletpoint_analyzer:
                print(f"  Running BulletpointAnalyzer (skipped in json mode for now)...")
                # TODO: Implement BulletpointAnalyzer for JSON storage
            
            # Apply curator operations handling via Interceptor
            import datetime
            from .utils.utils import get_section_slug
            for op in operations:
                if op.get('type') == 'ADD':
                    section_raw = op.get('section', 'general')
                    slug = get_section_slug(section_raw)
                    new_id = f"{slug}-{self.next_global_id:05d}"
                    self.next_global_id += 1
                    ...
                    if self.interceptor.request_approval(new_rule):
                        self.storage.add_rule(new_rule)
                        print(f"  Approved and added bullet {new_id} to section {section_raw}")
                    else:
                        print(f"  Rejected bullet {new_id}")
            
            # Refresh current playbook
            current_playbook = self.enhancer.get_relevant_playbook(question + "\n" + context)
